...
| IE-005 AtomS3 Lite proxy | **FLASHED; NETWORK ACCEPTANCE PENDING** | IE-004 plus owner USB flash/secrets gate | Atom is reserved at `.239` and the exact Kubernetes `/32:6053` path is deployed; UniFi rule, encrypted discovery, reboot/OTA recovery, and positive/negative reachability tests remain. |
