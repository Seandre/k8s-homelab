...
| IE-005 AtomS3 Lite proxy | OWNER GATE | IE-004 plus owner USB flash/secrets gate | Secret-free config and exact path are prepared. Owner reservation/USB flash precedes live rules; acceptance then requires encrypted discovery, reboot/OTA recovery, and exact `.21-.23` to `/32:6053` reachability while unrelated traffic stays blocked. |
