...
|---|---|---|---|---|---|---|---|---|---|
| Prometheus | Homelab monitoring | `http://kube-prometheus-stack-prometheus.monitoring.svc:9090` | HTTP REST; fixed PromQL catalog | Four Git-owned aggregate capacity/usage queries only; no admin, config, reload, write, arbitrary metric, or browser-supplied PromQL API | None | 5s loop / 3s | Last good normalized aggregate becomes stale after 45s | Return normalized scalar values and source state only | `IMPLEMENTED PREVIEW`: 30d retention on 50 GiB local-path PVC; host exporter targets await installation |
