...
This runbook restores the homelab control plane and GitOps-managed resources. Hardware, addresses, VM sizes, and storage live in the [Infrastructure Reference](../overview/infrastructure-reference.md). Persistent application data requires a separate backup and restore procedure.
...
Sealed Secrets adds a separate root of trust that Git cannot restore. Before using encrypted secrets, complete the key-backup and recovery exercise in [Optional 03: Sealed Secrets](../optional/sealed-secrets.md). During a rebuild, restore the sealing-key Secrets before starting or restarting the controller, then allow Argo CD to reconcile dependent `SealedSecret` resources.

## Rebuild Order

1. Install and configure `pve-01` and `vmdata` according to the [Infrastructure Reference](../overview/infrastructure-reference.md).
...
7. If Sealed Secrets is in use, restore its backed-up sealing-key Secrets into `kube-system` from the protected backup before the controller starts, following the rebuild order in [Optional 03: Sealed Secrets](../optional/sealed-secrets.md#perform-a-non-destructive-offline-recovery-exercise).
...
Recreate the internal DNS records listed in the [Infrastructure Reference](../overview/infrastructure-reference.md). All Kubernetes application hostnames point to `192.168.40.30`.
...
Restore persistent data only after the corresponding workloads and PVCs exist. KOReader-specific checks are in [Optional 02: KOReader Sync Runbook](../optional/koreader-sync-runbook.md).
...
Use [Operations 02: Troubleshooting](troubleshooting.md) when any layer fails.
