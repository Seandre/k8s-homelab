...
Complete this automation server before [Build 03](pve-02-and-bastion.md) or [Build 04](compact-okd.md).

The optional GUI is documented separately in [Optional 01: Utility Desktop and KOReader](../optional/utility-desktop-koreader.md). It is not required for later builds.
...
Copy the kubeconfig securely from your existing workstation. If you are rebuilding the cluster, follow the [k3s installation section of the Rebuild Runbook](../operations/rebuild-runbook.md#install-k3s) first. On `utility-01`, store the kubeconfig under `~/.kube`:
