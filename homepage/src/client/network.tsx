...
export function NetworkScreen({ bootstrap = healthyBootstrapFixture }: { bootstrap?: Bootstrap }) {
  ...
  return (
    <main className="dashboard" id="network">
      <section className="hero-row"><div><span className="panel-eyebrow">NETWORK / FIXTURE MODE</span><h1>Reachability and throughput</h1></div><div className="hero-state"><StateBadge severity={network.metadata.severity} label={network.metadata.severity} /><FreshnessLabel freshness={network.metadata.freshness} /></div></section>
      <div className="network-overview">
        <Panel className="network-box network-latency" title="Latency" eyebrow="READ-ONLY PROBES" severity={network.metadata.severity} freshness={network.metadata.freshness}><div className="metric-grid"><Metric label={`GATEWAY / ${network.gatewayLatencyProtocol ?? '—'}`} value={network.gatewayLatencyMs ?? '—'} unit="ms" detail="local gateway probe" /><Metric label={`INTERNET / ${network.internetLatencyProtocol ?? '—'}`} value={network.internetLatencyMs ?? '—'} unit="ms" detail="public HTTPS probe" /></div><div className="resource-graphs"><DotGraph label="GATEWAY" values={[1, 2, 1, Math.round(network.gatewayLatencyMs ?? 0)]} unit="ms" tone="download" /><DotGraph label="INTERNET" values={[12, 22, 16, Math.round(network.internetLatencyMs ?? 0)]} unit="ms" tone="upload" /></div></Panel>
        <Panel className="network-box network-throughput" title="Throughput" eyebrow="vmbr0 / CURRENT" severity="OK" freshness="CURRENT"><div className="resource-graphs network-graphs"><DotGraph label="DOWNLOAD" values={[220, 480, 610, 740, 620, 860]} unit="Mb/s" tone="download" height={3} /><DotGraph label="UPLOAD" values={[12, 23, 18, 31, 26, 39]} unit="Mb/s" tone="upload" height={3} /></div><span className="metric-detail">Host bridge traffic; no speed-test action is available.</span></Panel>
        ...
        <Panel className="network-box network-unifi" title="UniFi" eyebrow="CONTROLLER CAPABILITY" severity={network.unifi.metadata.severity} freshness={network.unifi.metadata.freshness}><div className="placeholder-state"><strong>NOT SUPPORTED</strong><span>{network.unifi.metadata.message}</span></div></Panel>
