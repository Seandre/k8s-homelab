...
import { ComponentGallery, DetailDrawer, DotGraph, FreshnessLabel, Metric, Panel, StateBadge } from './components.js';
...
export function OverviewScreen({ search, bootstrap = healthyBootstrapFixture }: { search: string; bootstrap?: Bootstrap }) {
  ...
  const primarySeries = data.timeSeries[0]!;
  ...
  return (
    <>
      <main className="dashboard" id="overview">
        <section className="hero-row"><div><span className="panel-eyebrow">OVERVIEW / FIXTURE MODE</span><h1>Operations at a glance</h1></div><div className="hero-state"><StateBadge severity={overview.globalSeverity} label={`${overview.globalSeverity} · ${overview.alerts.length} alert`} /><span>Last refresh {data.generatedAt.slice(11, 19)} UTC</span></div></section>
        ...
        <div className="pve-overview">
          {overview.proxmoxHosts.map((host, index) => <ProxmoxPanel key={host.id} host={host} index={index} expanded={expanded === host.id} onExpand={() => setExpanded(expanded === host.id ? null : host.id)} />)}
          ...
          <div className="pve-workload-row">
            <Panel className="network-box" title="Network" eyebrow="GATEWAY / INTERNET" severity={overview.network.metadata.severity} freshness={overview.network.metadata.freshness}><div className="metric-grid"><Metric label="GATEWAY" value={overview.network.gatewayLatencyMs ?? '—'} unit="ms" /><Metric label="INTERNET" value={overview.network.internetLatencyMs ?? '—'} unit="ms" /><Metric label="INGRESS VIP" value=".30" detail={overview.network.ingressVip ?? '—'} /></div><div className="resource-graphs"><DotGraph label="DOWNLOAD" values={[18, 31, 24, 42]} unit="Mb/s" tone="download" /><DotGraph label="UPLOAD" values={[8, 19, 15, 27]} unit="Mb/s" tone="upload" /></div></Panel>
            <Panel className="workload-box" title="Kubernetes" eyebrow="WORKLOAD / k3s" severity={primaryCluster.metadata.severity} freshness={primaryCluster.metadata.freshness} href="https://argocd.lab.seandre.dev"><div className="metric-grid"><Metric label="NODES" value="3 / 3" /><Metric label="WORKLOADS" value="18" /><Metric label="ALERTS" value="1" /></div><div className="resource-graphs"><DotGraph label="SCHEDULER" values={primarySeries.points.map((point) => point.value)} unit="%" tone="cpu" /></div></Panel>
      ...
      {expanded ? <DetailDrawer title={`${expanded} detail`} onClose={() => setExpanded(null)}><p>Keyboard and pointer expansion preserve the same normalized fixture data.</p><FreshnessLabel freshness="STALE" ageSeconds={42} /></DetailDrawer> : null}
    </>
  );
}
