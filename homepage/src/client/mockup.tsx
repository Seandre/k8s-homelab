      ...
      <main className="dashboard" id="overview">
        ...
        <div className="pve-overview">
          ...
          <div className="pve-workload-row">
            <Panel className="network-box" title="UDM Pro" eyebrow="NETWORK / UNIFI" severity={overview.network.unifi.metadata.severity} freshness={overview.network.unifi.metadata.freshness} href="https://unifi.ui.com"><div className="metric-grid"><Metric label="STATUS" value={overview.network.unifi.status ?? '—'} detail={overview.network.unifi.controller ?? 'UniFi Site Manager'} /><Metric label="WAN LATENCY" value={overview.network.lastSpeedTest.latencyMs ?? overview.network.internetLatencyMs ?? '—'} unit="ms" /><Metric label="GATEWAY" value={overview.network.gatewayLatencyMs ?? '—'} unit="ms" /></div><div className="metric-grid"><Metric label="WAN DOWN" value={overview.network.lastSpeedTest.downloadMbps ?? '—'} unit="Mb/s" /><Metric label="WAN UP" value={overview.network.lastSpeedTest.uploadMbps ?? '—'} unit="Mb/s" /><Metric label="INGRESS VIP" value={overview.network.ingressVip?.split('.').at(-1) ? `.${overview.network.ingressVip.split('.').at(-1)}` : '—'} detail={overview.network.ingressVip ?? ''} /></div></Panel>
