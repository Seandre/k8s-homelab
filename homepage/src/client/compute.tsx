...
export function ComputeScreen({ bootstrap = healthyBootstrapFixture }: { bootstrap?: Bootstrap }) {
  ...
  return (
    <main className="dashboard" id="compute">
      <section className="hero-row"><div><span className="panel-eyebrow">COMPUTE / FIXTURE MODE</span><h1>Hosts and clusters</h1></div></section>
      <section className="compute-section" aria-labelledby="proxmox-title"><div className="section-heading"><span className="panel-eyebrow">VIRTUALIZATION</span><h2 id="proxmox-title">Proxmox hosts</h2></div><div className="pve-overview compute-host-grid">{model.proxmoxHosts.map((host, index) => <ProxmoxPanel key={host.id} host={host} index={index} expanded={expanded === host.id} onExpand={() => setExpanded(expanded === host.id ? null : host.id)} />)}</div></section>
